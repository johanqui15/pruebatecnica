<div>
    <div id="map" style="height: 400px;"></div>
                ...
                function mostrarUbicacion() {
                    if (index < ubicaciones.locations.length) {
                        const ubicacion = ubicaciones.locations[index];
                        const marker = L.marker([ubicacion.latitud, ubicacion.longitud]).addTo(map);
                        marker.bindPopup(`<b>${ubicacion.nombre}</b><br>Latitud: ${ubicacion.latitud}<br>Longitud: ${ubicacion.longitud}`).openPopup();
                        map.panTo(new L.LatLng(ubicacion.latitud, ubicacion.longitud));
                        index++;
                    } else {
                ...
                const intervalo = setInterval(mostrarUbicacion, 1000);
                </script>
</div>